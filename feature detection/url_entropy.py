...
def shannon_entropy(url):
    """
    Calcula la entropia de Shannon de un url.
    """
    url = url.lower()
    total = len(url)
    counts = Counter(url) #convierte el url en un diccionario con la cantidad de apariciones de cada caracter
...
def relative_entropy(url, reference_probs):
    """
    Calcula la entropia relativa (KL divergence) entre
    la distribucion del url y una distribucion de referencia.
    """
    url = url.lower()
    total = len(url)
    counts = Counter(url) #convierte el url en un diccionario con la cantidad de apariciones de cada caracter
    ...
    for symbol, count in counts.items():
        p = count / total
        q = reference_probs.get(symbol, 0)
...
for domain in domains:
    ...
    kl = relative_entropy(domain, probabilities)

    print(f"Dominio: {domain}")
    print(f"  Entropia de Shannon: {h:.4f}")
    print(f"  Entropia relativa (KL): {kl:.4f}\n")
